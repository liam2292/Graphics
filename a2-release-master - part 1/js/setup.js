 ...
 */
function loadAndPlaceGLB(file, place, func) {
    ...
    loader.load(
        file,
        function (gltf) {
            //sets position scale and rotation
            gltf.scene.position.set(0, 5.5, -7);
            gltf.scene.scale.set(0.1, 0.1, 0.1);
            gltf.scene.rotation.y = Math.PI;
            //adds the model to the scene
            place.add(gltf.scene);

            //retrieve skeleton and bones for animation
            const skinnedMesh = gltf.scene.getObjectByProperty("type", "SkinnedMesh");
            if (!skinnedMesh) {
                console.error("No SkinnedMesh found in the Armadillo model!");
                return;
            }
            const skeleton = skinnedMesh.skeleton;
            //referencing bones in the left arm
            const leftShoulder = skeleton.bones[3]; 
            const leftUpperArm = skeleton.bones[4]; 
            const leftForearm = skeleton.bones[5];  
            const leftHand = skeleton.bones[6];      
            const leftHandIK = skeleton.bones[13];   
            //prints out name and index of each bone in the skeleton
