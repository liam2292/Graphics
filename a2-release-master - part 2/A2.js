...
//part 2
// definines torus material and geometry adding it to the scene and setting visibility to false
...
function update() {
  ...
  if(leftShoulder && leftHandIK) {
    ...
    // when the ball is caught it will turn into a torus object
    if(distToBall < maxDist && angle < maxAngle){
      const localSpherePos = leftHandIK.parent.worldToLocal(spherePos.clone());
      // superimposes the torus onto the ball
      torus.position.copy(spherePos);
      const handOffset = new THREE.Vector3(-1.0, 0, 0);
      leftHandIK.position.copy(localSpherePos.add(handOffset));

      //Change visibility of the two objects give the appearence of swapping the super imposed objects.
      sphere.visible = false;
      torus.visible = true;
      isBallHeld = true;
    } else {
      //if the ball is held then it will be shrunk by 0.1 until it reaches 0.2
      if(isBallHeld) {
        if(sphere.scale.x > 0.2) {
          sphere.scale.subScalar(0.1);
          isBallHeld = false;
        }
      }
      //when the torus is out of range then reverts to shrunken ball
      torus.visible = false;
      sphere.visible = true; 
    }
    ikSolver.update();
